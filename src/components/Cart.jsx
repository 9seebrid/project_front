...
const Cart = () => {
  ...
  useEffect(() => {
    const fetchCarts = async () => {
      try {
        const response = await axios.get(`https://project.9seebird.site/get_cart/${userId}`);
        const { cartItems, diyItems } = response.data;
        const productRequests = cartItems.map((item) =>
          axios.get(`https://project.9seebird.site/get_detail_products/${item.product_id}`)
        );
  ...
  const quantityChange = async (userId, productId, productSize, newQuantity, cartItemId) => {
    try {
      ...
      // 수량 업데이트 요청
      await axios.post(`https://project.9seebird.site/update_quantity/${userId}`, requestBody);
  ...
  const noItem = async (userId, productId, productSize, cartItemId) => {
    try {
      ...
      if (isDiyItem) {
        // DIY 아이템 삭제 요청
        await axios.post(`https://project.9seebird.site/delete_item/${userId}`, { cartItemId });
      } else {
        // 일반 아이템 삭제 요청
        await axios.post(`https://project.9seebird.site/delete_item/${userId}`, { productId, productSize });
      }
      ...
      const productRequests = updatedCarts
        ...
        .map((item) => axios.get(`https://project.9seebird.site/get_detail_products/${item.product_id}`));
  ...
  const deleteAll = async () => {
    try {
      const response = await axios.post(`https://project.9seebird.site/delete_all/${userId}`);
  ...
  const deleteCheckedItems = async () => {
    try {
      ...
      // 서버에 삭제 요청 전송
      await axios.post(`https://project.9seebird.site/delete_checked_items/${userId}`, { items: itemsToDelete });

      // 삭제 후 장바구니 데이터 가져오기
      const updatedCartResponse = await axios.get(`https://project.9seebird.site/get_cart/${userId}`);
