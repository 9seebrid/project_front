...
const DiyItem = () => {
  ...
  useEffect(() => {
    axios.get(`https://project.9seebird.site/get_product`).then((response) => {
    ...
    if (existingItem) {
      ...
      if (shouldUpdate) {
        try {
          ...
          await axios.post(
            `https://project.9seebird.site/update_diy_quantity/${userId}`,
            {
      ...
      try {
        await axios.post(`https://project.9seebird.site/add_Diyitem/${userId}`, cartItem, {
          headers: { 'Content-Type': 'application/json' },
        });
        alert('장바구니에 추가되었습니다!');
